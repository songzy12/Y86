...
instr_t instruction_set[] = 
{
    ...
    {"rmexchange", HPACK(I_RMEXCHANGE, F_NONE), 6, R_ARG, 1, 1, M_ARG, 1, 0 },
...
mem_t init_mem2(int len)
{
    ...
    key = ftok(".", 1);
    len = ((len+BPL-1)/BPL)*BPL;
    shm_id = shmget(key, len, IPC_CREAT | 0600);
    result->len = len;
    result->contents = (byte_t *) shmat(shm_id, NULL, !SHM_RDONLY);
    return result;
}
...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	(hi0 == I_RRMOVL || hi0 == I_ALU || hi0 == I_PUSHL ||
	 hi0 == I_POPL || hi0 == I_IRMOVL || hi0 == I_RMMOVL || hi0== I_RMEXCHANGE ||
    ...
    need_imm =
	(hi0 == I_IRMOVL || hi0 == I_RMMOVL || hi0==I_RMEXCHANGE || hi0 == I_MRMOVL ||
