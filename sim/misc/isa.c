...
instr_t instruction_set[] = 
{
    ...
    //{".stringz", 0x00, }
    {"wrint", HPACK(I_EXTEND, E_WRINT), 2, R_ARG, 1, 1, NO_ARG, 0, 0},
    {"rdint", HPACK(I_EXTEND, E_RDINT), 2, R_ARG, 1, 1, NO_ARG, 0, 0},
    {"wrch", HPACK(I_EXTEND, E_WRCH), 2, R_ARG, 1, 1, NO_ARG, 0, 0},
    {"rdch", HPACK(I_EXTEND, E_RDCH), 2, R_ARG, 1, 1, NO_ARG, 0, 0},
    {"wrstr", HPACK(I_EXTEND, E_WRSTR), 2, R_ARG, 1, 1, NO_ARG, 0, 0},
...
void do_extension(ext_t lo0, state_ptr s, reg_id_t lo1)
{
    word_t argA = get_reg_val(s->r, lo1);
    //int tmp;
    char ctmp;
    switch(lo0)
    {
...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	(hi0 == I_RRMOVL || hi0 == I_ALU || hi0 == I_PUSHL ||
	 ...
	 hi0 == I_MRMOVL || hi0 == I_IADDL || hi0 == I_EXTEND );
